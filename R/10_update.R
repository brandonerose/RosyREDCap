...
update_DB <- function(
  ...
  if(force){
    ...
    DB$data_extract <- DB %>% get_redcap_data(labelled = labelled)
    ...
    if(will_update){
      ...
      if(length(deleted_records)>0){
        ...
        DB$data_extract <- remove_records_from_list(data_list = DB$data_extract,records = deleted_records,silent = T)
      }
      ...
      DB$summary$all_records$last_api_call[which(DB$summary$all_records[[DB$redcap$id_col]]%in%IDs)] <-
        DB$internals$last_data_update <-
        Sys.time()
      DB$data_extract <- remove_records_from_list(data_list = DB$data_extract,records = IDs,silent = T)
      for(TABLE in names(data_list)){
        DB$data_extract[[TABLE]] <- DB$data_extract[[TABLE]] %>% dplyr::bind_rows(data_list[[TABLE]])
      }
